...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const populatePageContent = () => {
        ...
        const creator = settings.apiSettings?.creator || 'Alpiann';

        setPageContent(DOM.pageTitle, settings.name, "Alpian UI Api's");
        setPageContent(DOM.wm, `© ${currentYear} ${creator}. Yuk ikutin salurannya di atas.`);
        setPageContent(DOM.appName, settings.name, "Alpian API");
        setPageContent(DOM.sideNavName, settings.name || "API");
        setPageContent(DOM.versionBadge, settings.version, "v2.0");
        ...
        if (DOM.apiLinksContainer) {
            ...
            const defaultLinks = [{ url: "https://whatsapp.com/channel/0029VbB0x9f4dTnIDQIMee31", name: "Cek Update Terbarunya", icon: "fab fa-whatsapp" }];
